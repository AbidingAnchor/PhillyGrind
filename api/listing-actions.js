...
function previewCommentText(value) {
  const collapsed = String(value || '').replace(/\s+/g, ' ').trim();
  if (collapsed.length <= COMMENT_PREVIEW_LENGTH) return collapsed;
  return `${collapsed.slice(0, COMMENT_PREVIEW_LENGTH).trim()}…`;
}
...
export default async function handler(req, res) {
  ...
  if (action === 'comment-on-post') {
    ...
    try {
      ...
      console.error('[listing-actions] comment-on-post failed:', error);
