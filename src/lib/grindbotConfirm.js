const TICKET_OFFER_PATTERNS = [
  ...
  // Buried / conditional offers at the end of troubleshooting replies
...
export function getPendingTicketHint(messages) {
  const chat = normalizeChatMessages(messages);
  for (let index = chat.length - 1; index >= 0; index -= 1) {
    const message = chat[index];
    if (message.role !== 'assistant') continue;
    if (message.meta?.awaitingTicketConfirm) {
      return {
        awaitingTicketConfirm: true,
        ticketDraft: message.meta.ticketDraft || buildTicketDraft(chat.slice(0, index)),
      };
    }
    if (message.kind === 'ticket_offer') {
      return {
        awaitingTicketConfirm: true,
        ticketDraft: message.meta?.ticketDraft || buildTicketDraft(chat.slice(0, index)),
      };
    }
    if (isTicketOfferMessage(message.content)) {
      return {
        awaitingTicketConfirm: true,
        ticketDraft: buildTicketDraft(chat.slice(0, index)),
      };
    }
    break;
  }

  if (chat.length >= 2 && chat[chat.length - 1].role === 'user' && isDirectTicketRequest(chat[chat.length - 1].content)) {
    return {
      awaitingTicketConfirm: true,
      ticketDraft: buildTicketDraft(chat.slice(0, -1)),
    };
  }
